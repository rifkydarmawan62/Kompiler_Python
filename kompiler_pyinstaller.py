...
if system() in ("Windows", "Darwin"):
    ...
    try:
        run("pyinstaller --version", shell = True, check = True)
        ...
        if system() == "Windows":
            ...
            try:
                run(PERINTAH_INSTALASI, shell = True, check = True)
            except CalledProcessError:
                ...
                if direktori_sementara:
                    ...
                    while True:
                        ...
                        if direktori_import:
                            ...
                            perintah_direktori_import += f"--paths \"{direktori_import} "
                        else:
                            break
                    if perintah_direktori_import != "":
                        perintah += perintah_direktori_import
                    if system() in ["Windows", "Darwin"]:
                        if askyesno(title = "Pilih Mode Jendela atau Konsol", message = "Ya untuk mode jendela\nTidak untuk mode konsol"):
                            perintah += "--windowed "
                        else:
                            perintah += "--console "
                        perintah_file_ikon : str = ""
                        while True:
                            print(f"{Fore.LIGHTBLUE_EX}Tekan Alt + Tab untuk membuka jendela baru{Fore.RESET}")
                            ...
                            if file_ikon:
                                ...
                                perintah_file_ikon += f"--icon \"{file_ikon}\" "
                            else:
                                break
                        if perintah_file_ikon != "":
                            perintah += perintah_file_ikon
                        ...
                        if system() == "Windows":
                            ...
                            while True:
                                ...
                                if FILE_SUMBER_DAYA:
                                    ...
                                    perintah_file_sumber_daya += f"--resource \"{FILE_SUMBER_DAYA}\" * * * "
                                else:
                                    break
                            if perintah_file_sumber_daya != "":
                                perintah += perintah_file_sumber_daya
